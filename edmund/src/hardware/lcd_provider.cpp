  ...
  namespace Hardware {
    ...
    void LcdProvider::PrintNumberLarge(int x, int y, u_int value, uint16_t color, int length) const {
      ...
      for (int i = 5; remainingValue >= 0 && i >= 0; i--) // 16bits uint, 5 digits max
      {
        ...
        remainingValue %= p;
        if (currentDigit > 10 || (currentDigit == 0 && printedDigitCount == 0 && i >= length))
          continue;
        lcd->drawChar(x + (printedDigitCount * 8), y + 10, (char)(currentDigit + ((int)'0')), color, !color, 1);
        printedDigitCount += 1;
      }
    }
    ...
    void LcdProvider::PrintNumberSmall(int x_pos, int y_pos, int value, uint16_t color, int length) const {
      ...
      for (int i = 1; i <= maxRange; i++) {
        ...
        if (!(currentDigit == 0 && i == 3)) // don't print on leading 0 for hundred
          lcd->drawBitmap(x_pos + (char_w * ((length - printedDigitCount) - 1)) - (i < 3 && length == 3), y_pos, Edmund::UI::Resources::numericFont[currentDigit], char_w, char_h, color);
        printedDigitCount += 1;
      }
    }
